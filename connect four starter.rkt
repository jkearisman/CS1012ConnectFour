...
;; valid is defined as: located on the board
(define (get-valid-neighboors c) 
           (local
             [(define x (coor-x c))
              (define y (coor-y c))
              (define lst (list -1 0 1))
              ;; this bit below is recursive, it generates all combinations of two numbers in -1,0,1. i.e (-1,-1),(-1,0)...(1,1)
              ;; it adds these comboa to the x y coordinates ofthe given coordinate to determine all possible neighboors
              ;; note that this includes the coordinates of the checker itself
              (define (coor-list lst1 lst2) 
                (cond
                  [(empty? lst1) empty]
                  [(empty? lst2) (coor-list (rest lst1) lst)]
                  [else (cons (make-coor (+ x (first lst1)) (+ y (first lst2))) (coor-list lst1 (rest lst2)))]))
              (define (valid? c)  ;; checks to see if the given coordinate is valid for our board
                (cond 
                  [(< (coor-x c) 0) false]
                  [(< (coor-y c) 0) false]
...
(define (evaluation-function state)
  (local[ 
         (define (eval-coor c) ;; scores the coordinate with a super-secret process
           (local [(define this-checker (get-checker c state))
                   (define (score c1)
                     (local [(define other-checker (get-checker c1 state))]
                       (* (cond
                            [(= other-checker this-checker) ALLY-SCORE] ;; scores surrounding checkers, allies are worth a certain amount
                            [(= other-checker BLANK) BLANK-SCORE]       ;; so are blank spaces
                            [else ENEMY-SCORE])                         ;; enemies also add score
                          (if (= this-checker RED)
                              1
                              -1))))]
             (if (= this-checker BLANK)
                 0
                 (foldr + 0 (map score (get-valid-neighboors c))))))
         (define (one-list lol) ;; simplifies board into a single list, in order to simplify other areas of the code
           (if (empty? lol) 
               empty
               (cons (first lol) (one-list (rest lol)))))
         (define (build-list-coor x y)
           (cond
             [(>= y ROWS) empty]
             [(>= x COLUMNS) (build-list-coor 0 (add1 y))]
             [else (cons (make-coor x y) (build-list-coor (add1 x) y))]))]
    (if (check-win? state)
        (if (= (world-state-whose-turn state) RED) -99999999999 99999999999) ;; makes sure there are no winning options available
        (foldr + 0 (map eval-coor (one-list (build-list-coor 0 0)))))))
